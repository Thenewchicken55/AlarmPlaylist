...
export default function CreatePlaylistModal({ open, onClose }: CreatePlaylistModalProps) {
  ...
  async function handleFetch() {
    ...
    try {
      const result = await fetchYouTubePlaylist(playlistId, (loaded, total) => {
        if (total > 50) toast.loading(`Fetching tracks… ${loaded}/${total}`, { id: toastId })
      })
      setFetchedTracks(result.tracks)
      setPlaylistTitle(result.title)
      setName(result.title)
      toast.success(`Found ${result.tracks.length} tracks`, { id: toastId })
    } catch (err) {
      toast.error(err instanceof Error ? err.message : 'Failed to fetch playlist', { id: toastId })
    } finally {
      setLoading(false)
    }
  }
